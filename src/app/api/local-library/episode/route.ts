...
/**
 * DELETE /api/local-library/episode?source=...&id=...&episode=10
 */
export async function DELETE(request: NextRequest) {
  try {
    ...
    const episodeStr = padEpisode(episode);
    const prefix = `episode_${episodeStr}`;
    ...
    console.error('[Local Library Episode API] 删除单集失败:', error);
