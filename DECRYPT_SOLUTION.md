...
2. **Python 解密失败**: "Padding is incorrect"
...
- 尝试多种密钥生成方式（直接 UTF-8、MD5、SHA256）
- 尝试多种 IV 生成方式（UTF-8 编码、十六进制解析+填充）
- 自动处理填充错误
...
1. **密钥生成**:
   ...
   - 如果长度不是 16/24/32 字节，CryptoJS 会使用 MD5 哈希
   - Python: `key = hashlib.md5(key_str.encode('utf-8')).digest()`

2. **IV 生成**:

   - `iv_str = '2F131BE91247866E'`
   - `Utf8.parse()` 会将字符串转换为 UTF-8 字节数组
   - 16 个字符 = 16 字节（每个字符 1 字节）
...
**A**: 解密后的 URL 应该以 `http` 开头，并且可能包含 `m3u8` 或 `api/m3u8`。
...
- `final_direct_parser.py` - 基础 Python 解密方案
- `final_direct_parser_v2.py` - 改进的 Python 解密方案（尝试更多组合）
